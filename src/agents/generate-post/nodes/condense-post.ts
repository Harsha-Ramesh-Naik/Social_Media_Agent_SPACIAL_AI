...
import { filterLinksForPostContent, removeUrls, getMaxTwitterBodyLength } from "../../utils.js";
import {
  REFLECTIONS_PROMPT,
...
Given the marketing report, link, rules and structure, please condense the post down to roughly {maxTwitterLength} characters (not including the link). The original post was {originalPostLength} characters long.
...
export async function condensePost(
  ...
  const condensed = parseGeneration(condensePostResponse.content as string);
  return {
    post: condensed,
    twitterPost: condensed,
