...
\`\`\``;
  ...
  const instagramStr = instagramCaption
    ? `\n\nInstagram caption:
\`\`\`
${instagramCaption}
\`\`\``
...
>(state: State, config: LangGraphRunnableConfig): Promise<Update> {
  ...
  const linkedinPost = state.linkedinPost || state.post;
  const twitterPost = state.twitterPost || state.post;
  const instagramPost = (state as any).instagramPost || undefined;
