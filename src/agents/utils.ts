...
/**
 * Returns true if Arcade authentication should be used for Twitter.
 * Falls back to the legacy global flag for backward compatibility.
 */
export function useArcadeAuthForTwitter(): boolean {
  if (process.env.USE_ARCADE_AUTH_TWITTER !== undefined) {
    return process.env.USE_ARCADE_AUTH_TWITTER === "true";
  }
  return useArcadeAuth();
}

/**
 * Returns true if Arcade authentication should be used for LinkedIn.
 * Falls back to the legacy global flag for backward compatibility.
 */
export function useArcadeAuthForLinkedIn(): boolean {
  if (process.env.USE_ARCADE_AUTH_LINKEDIN !== undefined) {
    return process.env.USE_ARCADE_AUTH_LINKEDIN === "true";
  }
  return useArcadeAuth();
}
...
/**
 * Calculates the maximum allowed character length for the main Twitter post body text
 * (excluding signature prefix, link, and spacing) to ensure it never gets truncated
 * on upload.
 */
export function getMaxTwitterBodyLength(): number {
...
/**
 * Log URL content extraction errors to URL_checker_Log file in CWD
 */
export function logUrlError(url: string, error: any) {
  const logMessage = `[${new Date().toISOString()}] URL: ${url} | Error: ${error instanceof Error ? error.message : String(error)}\n`;
  try {
    fs.appendFileSync(path.resolve(process.cwd(), "URL_checker_Log"), logMessage);
  } catch (e) {
    console.error("Failed to write to URL_checker_Log", e);
  }
}
