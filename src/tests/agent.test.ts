...
import {
  getPlatformPost,
  parseGeneration,
} from "../agents/generate-post/nodes/generate-post/utils.js";
...
describe("platform-native post parsing", () => {
  it("extracts LinkedIn and Twitter variants from platform-native XML tags", () => {
    const raw = `
      <thinking>Need a platform-native variation</thinking>
      <linkedin-post>LinkedIn copy with hashtags #AI</linkedin-post>
      <twitter-post>Twitter copy with punchy hook</twitter-post>
    `;

    expect(parseGeneration(raw)).toBe(
      "LinkedIn copy with hashtags #AI\n\n---\n\nTwitter copy with punchy hook",
    );
    expect(getPlatformPost(raw, "linkedin")).toBe(
      "LinkedIn copy with hashtags #AI",
    );
    expect(getPlatformPost(raw, "twitter")).toBe(
      "Twitter copy with punchy hook",
    );
  });

  it("falls back to legacy <post> tags when platform-native tags are absent", () => {
    const raw = `
      <thinking>legacy</thinking>
      <post>Legacy single post copy</post>
    `;

    expect(parseGeneration(raw)).toBe("Legacy single post copy");
    expect(getPlatformPost(raw, "linkedin")).toBe(raw.trim());
    expect(getPlatformPost(raw, "twitter")).toBe(raw.trim());
  });
});
